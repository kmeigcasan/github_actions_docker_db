...
class Wall extends Model {
    ...
    {
        const query = `SELECT messages.id as message_id, CONCAT(messenger.first_name, ' ', messenger.last_name) as message_sender_name, message as message_content, messages.created_at as message_date, 
            CONCAT(commenter.first_name, ' ', commenter.last_name) as comment_sender_name, comment as comment_content, comments.created_at as comment_date FROM messages 
    ...
    {
        if(!FormHelper.hasMissingFields(req, res)){
            const query = "INSERT INTO comments(user_id, message_id, comment) VALUES (?, ?, ?)";
